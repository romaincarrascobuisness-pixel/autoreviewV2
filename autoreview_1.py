#!/usr/bin/env python3
"""
AutoReview - Version stable Railway
"""
...
def get_credentials():
    ...
    if not creds or not creds.valid:
        ...
        print("⚠️  Nouveau token généré — mets à jour GOOGLE_TOKEN sur Railway :")
        print(creds.to_json())
...
def get_all_locations(creds):
    try:
        ...
        print(f"❌ Erreur get_locations : {e}")
        return []
...
def generate_reply(client_config, review_text, star_rating, reviewer_name):
    try:
        ...
        if star_rating >= 4:
            tone = "remercie chaleureusement et invite à revenir"
        elif star_rating == 3:
            tone = "remercie, reconnais les points d'amélioration, reste positif"
        else:
            tone = "excuse-toi sincèrement, propose de recontacter"

        prompt = f"""Tu es le gérant de {client_config['business_name']}, un {client_config['business_type']}.
Infos : {client_config.get('business_description', '')}
Style : {client_config['business_tone']}.
Réponds à cet avis : Note {star_rating}/5, Client : {reviewer_name}, Avis : "{review_text}"
Règles : français, max 3 phrases, {tone}, signe "L'équipe de {client_config['business_name']}", réponse uniquement."""
...
def main():
    ...
    while True:
        try:
            ...
            if not locations:
                print("⚠️  Aucun établissement trouvé — nouvelle tentative dans 5 min")
            else:
                print(f"✅ {len(locations)} établissement(s) trouvé(s)")
                for location in locations:
                    ...
                    if not client_config:
                        print(f"⏭️  [{location_title}] non configuré — ignoré")
                        continue
                    ...
                    for review in reviews:
                        ...
                        is_old = False
                        try:
                            rt = datetime.fromisoformat(review.get("createTime","").replace("Z","+00:00"))
                            if rt < start_time:
                                is_old = True
                        except:
                            pass
                        ...
                        if reply:
                            ...
                            if success:
                                ...
                                delay = random.randint(DELAY_OLD_MIN, DELAY_OLD_MAX) if is_old else DELAY_NEW
                                print(f"⏳ Pause {delay}s...")
                                time.sleep(delay)

            print(f"✓ Cycle terminé — prochain dans {CHECK_INTERVAL//60} min\n")
